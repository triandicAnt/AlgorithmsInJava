# Algorithms In Java
java codes

```
.
├── A
│   ├── AddTwoNumbers.java
│   ├── BinartTreeRightView.java
│   ├── BuySell1.java
│   ├── Codec.java
│   ├── GrayCode.java
│   ├── KthLargest.java
│   ├── LCABST.java
│   ├── LCA.java
│   ├── LetterCombination.java
│   ├── LinkedListCycle.java
│   ├── LinkedListIntersect.java
│   ├── LongestPalindromicSubstring.java
│   ├── LongestSubstring.java
│   ├── LRUCache.java
│   ├── MergeKSorted.java
│   ├── MergeTwoList.java
│   ├── MinStack.java
│   ├── NumberOfIslands.java
│   ├── PalindromeLinkedList.java
│   ├── ProductArray.java
│   ├── RandomPointers.java
│   ├── ReverseLinkedList.java
│   ├── ReverseWords.java
│   ├── RotateMatrix.java
│   ├── Search2D.java
│   ├── Subsets.java
│   ├── ThreeSum.java
│   ├── TrapWater.java
│   ├── TwoSum.java
│   ├── ValidAnagram.java
│   ├── ValidateBST.java
│   ├── ValidParanthesis.java
│   ├── WordBreakII.java
│   ├── WordBreak.java
│   └── WordLadder.java
├── array
│   ├── ArrayProblems.java
│   ├── CountComp.java
│   ├── CountCoPrimes.java
│   ├── CountPairs.java
│   ├── EquilibriumElement.java
│   ├── FindPairWithleastDiff.java
│   ├── HistogramArea.java
│   ├── LongestNonDecSeq.java
│   ├── MaximumProduct.java
│   ├── MaximumSumSubArray.java
│   ├── MedianOfSortedArray.java
│   ├── MergeSortedArrayInPlace.java
│   ├── PeakElement.java
│   ├── RemoveDup.java
│   ├── RemoveDuplicatesArray.java
│   ├── SearchInRotatedArray.java
│   ├── SelectAlgorithm.java
│   ├── SubArrayWithGivenSum.java
│   └── ToeplizMatrix.java
├── backtracking
│   ├── AllSubsetChars.java
│   ├── RatInMaze.java
│   └── StringPermutation.java
├── bits
│   ├── BitsToFlip.java
│   └── CountBits.java
├── com
│   ├── datastructure
│   │   ├── AdjacencyList.java
│   │   ├── bst
│   │   │   ├── BinaryTree.java
│   │   │   ├── BinaryTreeProblems.java
│   │   │   ├── BTNode.java
│   │   │   ├── BuildTreeWithInPreOrder.java
│   │   │   ├── DeleteNode.java
│   │   │   ├── DisgonalSumBT.java
│   │   │   ├── FlattenTree.java
│   │   │   ├── InOrderSuccessor.java
│   │   │   ├── LeafToLeafMaxSum.java
│   │   │   ├── LongestPath.java
│   │   │   ├── MorrisInOrder.java
│   │   │   ├── NumberOfBST.java
│   │   │   ├── NumberOfBSTs.java
│   │   │   ├── PrintBoundaryTree.java
│   │   │   ├── PrintLeftmostRightmostNode.java
│   │   │   └── RootToLeafSumBT.java
│   │   ├── HeapSort.java
│   │   ├── linkedlist
│   │   │   ├── AddNumsLinkedList.java
│   │   │   ├── LinkedList.java
│   │   │   └── MyLinkedList.java
│   │   ├── QTest.java
│   │   ├── QuickSort.java
│   │   └── tries
│   │       ├── Trie2.java
│   │       ├── TrieClass.java
│   │       └── Trie.java
│   ├── graph
│   │   ├── CountSCC.java
│   │   ├── DisjointSet.java
│   │   ├── Graph.java
│   │   ├── GraphTraversals.java
│   │   ├── GraphVertex.java
│   │   ├── StronglyConnectedComponents.java
│   │   └── TopologicalSort.java
│   └── thread
│       └── ProducerConsumer.java
├── dp
│   ├── CheckArthimeticProgression.java
│   ├── EditDistanceOfString.java
│   ├── MinimumCoinsToSum.java
│   ├── PossiblePathsInMatrix.java
│   ├── StoreWater.java
│   └── SubsetSumInArray.java
├── hackerrank
│   ├── Factory.java
│   ├── IpValidate.java
│   ├── PrintScoreYahoo.java
│   ├── ScannerTest.java
│   └── SearchTextYahoo.java
├── interviews
│   ├── ArrayHopper.java
│   ├── FindSentence.java
│   ├── GetSentenceFromPOS.java
│   ├── InterviewQuestions.java
│   ├── PrintSpiral.java
│   └── ReverseSentence.java
├── leetcode
│   ├── Atoi.java
│   ├── BestTimeToBuySellStock.java
│   ├── BinaryTreeZigZag.java
│   ├── BitwiseAND.java
│   ├── BulbSwitcher.java
│   ├── BuySellStockCoolDown.java
│   ├── CheckTriplets.java
│   ├── CombinationSumIII.java
│   ├── CombinationSumII.java
│   ├── CombinationSum.java
│   ├── ConstructInPost.java
│   ├── ConstructTreeInPre.java
│   ├── ContainerWithMostWater.java
│   ├── CountArithmeticP.java
│   ├── CountBits.java
│   ├── CountUniqueDigitNumber.java
│   ├── CourseSchedule1.java
│   ├── DeleteNode.java
│   ├── DetectCcleLinkedList.java
│   ├── EliminationGame.java
│   ├── FindMaxXorInArray.java
│   ├── FindMin.java
│   ├── FindPeak.java
│   ├── FlattenBST.java
│   ├── FourSum.java
│   ├── GasStation.java
│   ├── GrayCode.java
│   ├── GroupAnagram.java
│   ├── GuessNumberII.java
│   ├── HouseRobberIII.java
│   ├── HouseRobberII.java
│   ├── HouseRobberI.java
│   ├── InOrderTraversal.java
│   ├── InsertionSortLinkedList.java
│   ├── IntegerBreak.java
│   ├── IntegerReplacement.java
│   ├── IsSubsequence.java
│   ├── JumpGameII.java
│   ├── JumpGameI.java
│   ├── KPairSmallestSum.java
│   ├── KthLargestElement.java
│   ├── KthSmallestBST.java
│   ├── KthSmallestInMatrix.java
│   ├── LargestDivisibleSubset.java
│   ├── LCSBinaryTree.java
│   ├── LCSBst.java
│   ├── LengthOfLIS.java
│   ├── LFUCache.java
│   ├── LinkedListRandomNode.java
│   ├── ListNode.java
│   ├── ListNodePartition.java
│   ├── LongestAbsolutePath.java
│   ├── LongestCommonPrefix.java
│   ├── LongestNonRepeatingSubstring.java
│   ├── LongestPalindromicSubstring.java
│   ├── LongestSubstringWithKRepeat.java
│   ├── LRUCache.java
│   ├── MatchStickToSquare.java
│   ├── MaxProductWord.java
│   ├── MaxSumSubArray.java
│   ├── MedianOfSortedArrays.java
│   ├── MergeKLinkedList.java
│   ├── MinimumPathSumGrid.java
│   ├── MinSizeSubArraySum.java
│   ├── MissingNumber.java
│   ├── NextRightPointer.java
│   ├── NthUgly.java
│   ├── NumberOfIslands.java
│   ├── OddEvenList.java
│   ├── OnesAndZeros.java
│   ├── PalindromePartition.java
│   ├── PalindromicNumber.java
│   ├── PartitionEqualSubset.java
│   ├── PathSumII.java
│   ├── PerfectSquare.java
│   ├── PermutationII.java
│   ├── Permutations.java
│   ├── PhoneLettersCombination.java
│   ├── PreOrderTraversal.java
│   ├── ProdutExceptItself.java
│   ├── Q100.java
│   ├── Q101.java
│   ├── Q102.java
│   ├── Q104.java
│   ├── q107.java
│   ├── Q110.java
│   ├── Q111.java
│   ├── Q112.java
│   ├── Q116.java
│   ├── Q121.java
│   ├── Q122.java
│   ├── Q124.java
│   ├── Q125.java
│   ├── Q126.java
│   ├── Q128.java
│   ├── Q130.java
│   ├── Q133.java
│   ├── Q136.java
│   ├── Q138.java
│   ├── Q139.java
│   ├── Q13.java
│   ├── Q141.java
│   ├── Q143.java
│   ├── Q148.java
│   ├── Q150.java
│   ├── Q151.java
│   ├── Q152.java
│   ├── Q155.java
│   ├── Q160.java
│   ├── Q165.java
│   ├── Q166.java
│   ├── Q168.java
│   ├── Q169.java
│   ├── Q171.java
│   ├── Q173.java
│   ├── Q174.java
│   ├── Q179.java
│   ├── Q189.java
│   ├── Q190.java
│   ├── Q191.java
│   ├── Q19.java
│   ├── Q1.java
│   ├── Q203.java
│   ├── Q208.java
│   ├── Q210.java
│   ├── Q211.java
│   ├── Q212.java
│   ├── Q217.java
│   ├── Q218.java
│   ├── Q219.java
│   ├── Q21.java
│   ├── Q220.java
│   ├── Q221.java
│   ├── Q222.java
│   ├── Q226.java
│   ├── Q228.java
│   ├── Q229.java
│   ├── Q231.java
│   ├── Q232.java
│   ├── Q234.java
│   ├── Q237.java
│   ├── Q238.java
│   ├── Q239.java
│   ├── Q23.java
│   ├── Q242.java
│   ├── Q24.java
│   ├── Q257.java
│   ├── Q258.java
│   ├── Q25.java
│   ├── Q26.java
│   ├── Q270.java
│   ├── Q273.java
│   ├── Q276.java
│   ├── Q27.java
│   ├── Q283.java
│   ├── Q285.java
│   ├── Q287.java
│   ├── Q28.java
│   ├── Q292.java
│   ├── Q297.java
│   ├── Q29.java
│   ├── Q303.java
│   ├── Q304.java
│   ├── Q307.java
│   ├── Q31.java
│   ├── Q322.java
│   ├── Q326.java
│   ├── Q329.java
│   ├── Q332.java
│   ├── Q342.java
│   ├── Q344.java
│   ├── Q345.java
│   ├── Q348.java
│   ├── Q365.java
│   ├── Q371.java
│   ├── Q377.java
│   ├── Q387.java
│   ├── Q389.java
│   ├── Q390.java
│   ├── Q394.java
│   ├── Q402.java
│   ├── Q404.java
│   ├── Q407.java
│   ├── Q409.java
│   ├── Q414.java
│   ├── Q415.java
│   ├── Q417.java
│   ├── Q419.java
│   ├── Q42.java
│   ├── Q434.java
│   ├── Q43.java
│   ├── Q448.java
│   ├── Q452.java
│   ├── Q453.java
│   ├── Q455.java
│   ├── Q456.java
│   ├── Q459.java
│   ├── Q461.java
│   ├── Q463.java
│   ├── Q476.java
│   ├── Q485.java
│   ├── Q50.java
│   ├── Q516.java
│   ├── Q54.java
│   ├── Q56.java
│   ├── Q58.java
│   ├── Q5.java
│   ├── Q60.java
│   ├── Q61.java
│   ├── Q63.java
│   ├── Q69.java
│   ├── Q70.java
│   ├── Q71.java
│   ├── Q72.java
│   ├── Q79.java
│   ├── Q80.java
│   ├── Q83.java
│   ├── Q88.java
│   ├── Q91.java
│   ├── Q93.java
│   ├── Q97.java
│   ├── Q98.java
│   ├── RandomPick.java
│   ├── RemoveDuplicateSortedII.java
│   ├── RemoveDuplicateSortedLL.java
│   ├── RepeatDNASequence.java
│   ├── ReverseInteger.java
│   ├── ReverseLinkedListII.java
│   ├── ReverseLinkedListI.java
│   ├── RightViewBT.java
│   ├── RotateMatrix.java
│   ├── Search2dMatrix.java
│   ├── Search2DMatrix.java
│   ├── SearchForARange.java
│   ├── SearchInRotatedArrayII.java
│   ├── SearchInRotatedArray.java
│   ├── SearchInsert.java
│   ├── SetMatrixZero.java
│   ├── SingleNumberIII.java
│   ├── SingleNumberII.java
│   ├── SortColors.java
│   ├── SortedArrayToBST.java
│   ├── SortedListToBST.java
│   ├── SpiralMatrixII.java
│   ├── SubsetII.java
│   ├── Subsets.java
│   ├── SumOfLinkedList.java
│   ├── SumRootToLeafNode.java
│   ├── SuperPow.java
│   ├── SuperUgly.java
│   ├── ThreeSumClosest.java
│   ├── ThreeSumZero.java
│   ├── TopKFrequentElements.java
│   ├── TreeNode.java
│   ├── Triangle.java
│   ├── TwoSum.java
│   ├── TwoSumSorted.java
│   ├── UniqueBinaryTreesII.java
│   ├── UniquePathII.java
│   ├── UniquePathsI.java
│   ├── UniquePaths.java
│   ├── UniqueSubstrinInWrap.java
│   ├── ValidPerfectSquare.java
│   └── WiggleSubsequence.java
├── M
│   ├── AssignNext.java
│   ├── BestTimeBuyStock.java
│   ├── BTZigZag.java
│   ├── CountPrimes.java
│   ├── DeleteLinkedListNode.java
│   ├── DungeonGame.java
│   ├── ExcelNumberTitle.java
│   ├── ExcelSheetCol.java
│   ├── IntersectLinkedList.java
│   ├── KthLarget.java
│   ├── LevelOrder.java
│   ├── LinkedListCycle.java
│   ├── LongestPalindromicSubstring.java
│   ├── MaximumSubArray.java
│   ├── MediaOfSortedArray.java
│   ├── MergeSortedArray.java
│   ├── MinimumInRotated.java
│   ├── MissingNumber.java
│   ├── MyQueue.java
│   ├── NextRightPointerII.java
│   ├── NoOf1Bits.java
│   ├── NumberOfIslands.java
│   ├── NumberToWords.java
│   ├── ProductExceptItself.java
│   ├── Q101.java
│   ├── Q106.java
│   ├── Q112.java
│   ├── Q114.java
│   ├── Q124.java
│   ├── Q138.java
│   ├── Q162.java
│   ├── Q165.java
│   ├── Q173.java
│   ├── Q212.java
│   ├── Q270.java
│   ├── Q285.java
│   ├── Q28.java
│   ├── Q300.java
│   ├── Q348.java
│   ├── Q419.java
│   ├── Q46.java
│   ├── Q47.java
│   ├── Q4.java
│   ├── Q56.java
│   ├── Q71.java
│   ├── Q79.java
│   ├── Q91.java
│   ├── Q94.java
│   ├── Q98.java
│   ├── RemoveDuplicates.java
│   ├── ReverseNodesInKGrp.java
│   ├── ReverseWords.java
│   ├── RomanToInt.java
│   ├── RotateArray.java
│   ├── RotateMatrix.java
│   ├── SearchInRotated.java
│   ├── SerializeDeserialize.java
│   ├── SetMatrixZeros.java
│   ├── Skyline.java
│   ├── SortColors.java
│   ├── SpiralMatrix.java
│   ├── SwapNodesInPairs.java
│   ├── ThreeSum.java
│   └── Trie.java
├── random
│   ├── AddSubtractNumbers.java
│   ├── Bucket.java
│   ├── ClassA.java
│   ├── CommonElements.java
│   ├── Fibonacci.java
│   ├── FindOnes.java
│   ├── GenrateRandom.java
│   ├── IsItPossible.java
│   ├── PoweSet.java
│   ├── PrimeFactors.java
│   ├── RecursionProduct.java
│   ├── Shape.java
│   ├── SortZeroAndOnes.java
│   ├── TestClass1.java
│   └── ThreadC.java
├── recursion
│   └── AllSubsetWithSum.java
├── regex
│   ├── AncestorTree.java
│   ├── FamilyTree.java
│   ├── MatcherDemo.java
│   ├── RegexTestHarness.java
│   ├── RemoveSpecial.java
│   ├── StringSplit.java
│   └── ValidateDemo.java
└── string
    ├── AllPermutation.java
    ├── AllUniqCharCheck.java
    ├── ComparatorTest.java
    ├── CountPalin.java
    ├── DigitToWord.java
    ├── Fibonacci.java
    ├── GenerateParanthesis.java
    ├── KMP.java
    ├── LCS.java
    ├── LexicographicRotation.java
    ├── LongestNonRepeatingSubstring.java
    ├── LongestPalindromicSubsequence.java
    ├── LongestPalindromicSubstring.java
    ├── LongestRepeatingString.java
    ├── MatchPattern.java
    ├── MinimumSplitForPalindrome.java
    ├── PrintNonOverlappingNums.java
    ├── Q28.java
    ├── RearrangeCharacters.java
    ├── StringAnagram.java
    ├── StringChain.java
    ├── StringInterleaving.java
    ├── StringIntersect.java
    ├── StringReverse.java
    ├── UtopianTree.java
    └── WordBreakDP.java

20 directories, 487 files

```
